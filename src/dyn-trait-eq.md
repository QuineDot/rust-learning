...
Additionally, we'll rely on the relatively new ability of
[supertrait upcasting](dyn-trait-coercions.md#supertrait-upcasting)
to make things more ergonomic.  If your <abbr title="Minimum Supported Rust Version">MSRV</abbr>
doesn't allow for that, the example is still possible, but you'll need
to [supply your own upcasting ability.](http://localhost:3000/dyn-trait-combining.html#manual-supertrait-upcasting)
The bottom of this chapter shows what that would look like.
...
Before trait upcasting, we would use the "supertrait we can blanket implement"
pattern yet again.  But with modern Rust, we only need the supertrait bound
on `DynCompare`.
...
[`&dyn Any` is always actually `&(dyn Any + 'static)`.](dyn-elision-trait-bounds.md#the-static-case)
...
this example could be made more general; you will also need some other
approach to support upcasting.
...
Here we've utilized `dyn Any` upcasting to try and recover a
...
[Here's the final playground.](https://play.rust-lang.org/?version=stable&mode=debug&edition=2024&gist=d6a282a0bcf7e5ff858cdcb9410060fc)

## Comparison with manual supertrait upcasting

[Here's a playground which uses manual supertrait upcasting instead.](https://play.rust-lang.org/?version=stable&mode=debug&edition=2024&gist=a8eb5138fdab545e827dfc02d00ce961)
...
Depending on how your traits are used, it may or may not be worth it to provide
the `AsDynCompare` methods on top of built-in trait upcasting.  It need not
be a supertrait anymore, though, and can be done independently of the built-in
trait upcasting implementation explored above.
